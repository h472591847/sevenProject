...
class DealAction extends Action {
    ...
    /**
     *@todo    _initialize   初始化方法
     */
    public function _initialize(){
        $this->obj = M('Loan');//公用模型定义
        $this->page_size = 25;//公用分页数定义
        $this->uid = session('user_id');//当前会员ID 
    }
    ...
    public function Index(){
       ...
       foreach ($loan_list as $key => $value) {
            ...
            $end_time = $value['end_time'] * 24 * 3600;//天转换成秒        
            $retime = ($value['start_time'] + $end_time) - NOW_TIME;//当前剩余秒数 
            //若项目未开始,则算出距离开标剩余时间
            if($value['start_time'] > NOW_TIME){
              $retime = $value['start_time'] - NOW_TIME;
            }
            ...
            $value['retime'] = ($value['deal_status']==2)?0:$retime;//若满标则剩余天数清零
    ...
    public function DealContent(){
        ...
        $loan_ar['loan_rate'] = $loan_ar['loan_rate'] + $loan_ar['loan_add_rate'];
        $loan_ar['now_time'] = NOW_TIME;
    ...
    /**
     * @todo  ToInvestView  投资表单视图
     */
    public function ToInvestView(){
      $uid = $this->uid;
      $lid = I('id');
        $uid = session('user_id');
        // if(empty($uid))
    ...
    public function DoAddInvested(){    
        ...
        if(session('verify') != strtolower($code)){
            $data['url'] = U("Deal/ToInvestView",array('id'=>$lid));
            $data['info'] = '验证码输入错误，请重新输入';
            $data['status'] = 0;
            $this->ajaxReturn($data,'JSON');exit();
        }                        
...
////////////
////////////
$fp = fopen("./lock/DoAddInvestedLock.txt", "r+");
if(flock($fp, LOCK_EX)){  
////////////
////////////
...
////////////////            
            ...
            $data['url'] = U('Deal/DealContent',array('lid'=>$lid)); 
            $data['info'] = '投资成功!';
            $data['status'] = 1;
            $this->ajaxReturn($data,'JSON');exit();   
        }else{
