...
class userAction extends Action{
    ...
    public function DoRegister() {        
        ...
        $phone_code = I("post.code"); //手机验证码  
        $pid = I('pid',0,'intval'); //推广人ID                      
        $obj = M('web_user'); 
        if(empty($user_name))
           $this->error('用户名不能为空');
        if(empty($user_pwd))
        ...
        if ($insert = $obj->add($data)) {   
            ...
            // send_sms($phone, $phone_msg2);//发送免责短信
            // ++++++++++++++2015-04-09 10:27:08 update_by:liying++++++++++
    ...
    public function sendCode() { 
        ...
        if (empty($phone)) {
            $this->ajaxReturn('','手机号不能为空',0);exit();                     
        }
        /* 查看是否已存在此用户 */
        // $rs_phone = $obj->where(array('phone' => $phone))->find();
        // if ($rs_phone) {
        //     $this->ajaxReturn('','该手机号已注册，请重新填写手机号码',0);exit();              
        // }           
        $obj = M('temp_code');
        $where["phone"] = $phone;
        $where["status"] = 0;
        ...
        $msg = "验证码:$rnd,欢迎注册".$this->phone_title.",您的手机号码为$phone";                
        if($type == 1){
            $message = $rnd.',15';
    ...
    public function DoLogin() {
        ...
        if($referer_url != ''){
            ...
            if($ar_index[1] == 'Index/Index.html' || $referer_url == 'http://m.p2p222.com/' || $referer_url == 'http://m.p2p222.com/index.php'){
                $Prompt = 1;
            }
        }                                 
        //判断来路跳转
    ...
    public function DoResetPassword(){
        ...
        if($update === false){
            ...
            $msg_data['content'] = '【前台会员：'.$view['user_name'].'】重置密码成功';
    ...
    public function DoFindMail(){
        ...
        if($update !== false){           
            ...
            $msg_data['title'] = $msg_str.'认证邮箱';
            $msg_data['content'] = '【前台会员：'.$view['user_name'].'】'.$msg_str.'认证邮箱成功';
            $msg_data['user_id'] = $user_id;
            $msg_data['create_time'] = NOW_TIME;
            $msg_data['type'] = 1;//账户操作
            $msg_data['web_status'] = 1;//状态：前台操作
