...
class Aes{
	private $key; //密钥名称
	private $val; //密钥值
	//public function __construct($key="beijingrz-siyuan",$val="siyuan-rzky-2012"){
	public function __construct($key="beijingrz-ruiyun",$val="ruiyun-rzky-2014"){	
			$this->key = $key;
			$this->val = $val;
			$this->out =	'';
	}
	...
	public function aes_encode($data){
		return base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $this->key, $this->paddingPKCS7($data), MCRYPT_MODE_CBC,$this->val));
	}
	/**
	 * @todo   aes解密方法
	...
	public function aes_decode($data){
		return  mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $this->key,base64_decode($data), MCRYPT_MODE_CBC, $this->val);
	}
	/**
	 * @todo	siyuan加密方法
	 * @param   string or array $data
