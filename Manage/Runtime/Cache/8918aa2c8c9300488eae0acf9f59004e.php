...
<form action="<?php echo U('DealLog/index');?>" method="post">
    <tr>
      <td colspan="11" height="30">
        用户名：<input type="text" name="user_name" size="10" value="<?php echo ($page_param["user_name"]); ?>">&nbsp; &nbsp;       
        交易金额：<input type="text" name="money_start" size="5" value="<?php echo ($page_param["money_start"]); ?>"> - <input type="text" name="money_end" value="<?php echo ($page_param["money_end"]); ?>" size="5">&nbsp; &nbsp;         
        交易类型:<select name="type">
          <option value="" >--- 请选择 ---</option>                     
          <option value="1" <?php if($page_param["type"] == '1'): ?>selected="selected"<?php endif; ?>>充值</option> 
          <option value="2" <?php if($page_param["type"] == '2'): ?>selected="selected"<?php endif; ?>>投资</option> 
          <option value="3" <?php if($page_param["type"] == '3'): ?>selected="selected"<?php endif; ?>>提现</option> 
          <option value="4" <?php if($page_param["type"] == '4'): ?>selected="selected"<?php endif; ?>>还款本金</option> 
          <option value="5" <?php if($page_param["type"] == '5'): ?>selected="selected"<?php endif; ?>>还款利息</option> 
          <option value="6" <?php if($page_param["type"] == '6'): ?>selected="selected"<?php endif; ?>>管理费</option> 
          <option value="7" <?php if($page_param["type"] == '7'): ?>selected="selected"<?php endif; ?>>奖励金额</option>   
          <option value="8" <?php if($page_param["type"] == '8'): ?>selected="selected"<?php endif; ?>>系统操作</option>   
          <option value="9" <?php if($page_param["type"] == '9'): ?>selected="selected"<?php endif; ?>>邀请奖励</option>   
...
<?php if(is_array($info)): foreach($info as $key=>$v): ?><TR onMouseOver="this.style.background='#d0e2f5';" onMouseOut="this.style.background='#E3EFFB';">
    ...
    <td align="center">
        <?php echo (get_deal_log_type($v["type"])); ?>     
    </td>
    ...
    <td align="center"><?php if($v['remain_money'] != ''): ?>当<?php echo ($v["remain_money"]); else: ?>暂无<?php endif; ?></td>
    <td align="center"><?php if($v['remain_lock_money'] != ''): echo ($v["remain_lock_money"]); else: ?>暂无<?php endif; ?></td>
    <td align="center"><?php if($v['remain_bonus_money'] != ''): echo ($v["remain_bonus_money"]); else: ?>暂无<?php endif; ?></td>
    <td align="center">
    <?php if($v['pay_status'] == 1): ?><font style="color:#156430;font-weight:bold;">+</font>
    <?php else: ?>
    <font style="color:#F00;font-weight:bold;">-</font><?php endif; echo (format_price($v["money"])); ?></td>
    <td align="center"><a href="<?php echo U('DealLog/index',array('uid'=>$v['uid']));?>"><?php echo ($v["user_name"]); ?></a></td>
    <td align="center">
    <?php if($uid != ''): if($v['is_label'] == 0): ?><button  onClick="if (confirm('确定要设为已读状态吗？')){ this.form.action='<?php echo U('DealLog/IsLabel',array('uid'=>$uid,'id'=>$v['id'],'is_label'=>1));?>'; }else{ return false; }">未标记</button>
       <?php else: ?>
  ...
  <TR>
    ...
    <TD align="center" class="hs12"><input type="submit" value="确认删除" onClick="if (confirm('确定要删除吗？\r一旦删除无法恢复数据')){ this.form.action='<?php echo U('DealLog/InfoDelete');?>'; }else{ return false; }"></TD>
    </TR></form>
  </TABLE>
</BODY></HTML>