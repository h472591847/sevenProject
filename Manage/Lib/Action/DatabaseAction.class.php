...
class DatabaseAction extends CommonAction{
	...
	{   
		$db_back_dir = $_SERVER['DOCUMENT_ROOT'].__ROOT__."/Public/DbBackup/";      
	...
	{
	 			//开始写入sql脚本
				$filepath = $_SERVER['DOCUMENT_ROOT'].__ROOT__."/Public/DbBackup/".$filebase_name."/";   //导出的目录	 
	...
	{        
		...
		$db_back_dir = $_SERVER['DOCUMENT_ROOT'].__ROOT__."/Public/DbBackup/".$groupname."/";
		 	 ...
		 	 {  
		  		...
		  		if	(substr($file,$typelen)==$type)  
		  		{
		  			...
		  			if($fileIdx)
		  			{
			 	 		...
			 	 		(
			 	 			'filename'=>$file,
			 	 			'filedate'=>date('Y-m-d H:i:s',$file_only_name)
			 	 		);
		  			}
		  			else 
		  			{
